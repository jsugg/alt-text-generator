...
async function publishPagesBranch({
  ...
  const worktreeDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'publish-pages-branch-'));
...
async function main() {
  ...
  const result = await publishPagesBranch(args);
