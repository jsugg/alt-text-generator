...
module.exports = (serverLogger) => {
   ...
   */
  apiRouter.get(['/api/ping', '/api/v1/ping'], (req, res) => {
   ...
   *         description: Server error
   */
  apiRouter.get(['/api/health', '/api/v1/health'], health);
  ...
  apiRouter.get(
    ...
    async (req, res) => {
      ...
      req.log.logger.info(`${req.log.startTime} Request received`);
      req.log(req, res);
      let decodedImageURI = decodeURIComponent(req.query.image_source);
      if (decodedImageURI.includes('?')) {
        [decodedImageURI] = decodedImageURI.split('?');
      }
      const imageSource = decodedImageURI;
      const { model } = req.query;
      req.log.logger.info(
        `Model: ${model}, imageSource: ${imageSource}`,
      );

      if (
        !imageSource || !model || !validateUrl(imageSource)
      ) {
        res.status(400).json({
          error:
